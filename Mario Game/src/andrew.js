//Adding other scenes, make new json file with all scenes??
//game.state.add("Bahen", Bahen.js)
...
Mario_Game.test1.prototype = {
  preload: function () {
    ...
    game.load.spritesheet(
      "astronaut",
      "./assets/frosh_astronaut64x64.png",
      64,
      64
    );
    //~~~~~~~~~~~~~~~~~~~
    ...
    game.load.spritesheet(
      "big_purple_player",
      "./assets/Big_Main_SpritePowerup2.png",
      32,
      64
    );
  ...
  create: function () {
    ...
    //variables for door placement, door.position.x does not work
    doorx = 1000;
    doory = 465;
    ...
    player.animations.add(
      "left_blink",
      [10, 20, 9, 20, 8, 20, 10, 20, 7, 20, 6, 20, 10, 20],
      10,
      true
    );
    player.animations.add(
      "right_blink",
      [0, 20, 1, 20, 2, 20, 0, 20, 3, 20, 4, 20, 0, 20],
      10,
      true
    );
  ...
  update: function () {
    ...
    game.physics.arcade.collide(
      fireballs,
      enemy,
      function enemyKill(fireballs, enemy) {
        enemy.kill();
        fireballs.kill();
      },
      null,
    ...
    //Moving to Different State *DOOR*
    //Placed before jump function
    if (player.position.x >= doorx - 20 && player.position.x <= doorx + 20) {
      //20 offset to account for slight error in player placement
...
function powerUp_ingest(player, powerUp) {
  ...
  if (
    powerUpHierarchy[player.currentState] <=
    powerUpHierarchy[powerUp.power_type]
  ) {
    ...
    if (powerUp.power_type == "fireflower") {
      ...
      //Makes player have coffee powerup when they eat coffee
      player.loadTexture("big_purple_player");
      ...
      //Make player have bubble tea powerup
      player.loadTexture("big_purple_player");
      game.time.events.add(
        10000,
        function (player) {
